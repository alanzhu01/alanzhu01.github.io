...
import { clamp, roundInt, onBlurOrEnter } from "../utils/continuous.js";
...
function makeLabeledInput({ label, name, step = "any", tip = "" }) {
  ...
  onBlurOrEnter(input, () => {
    normalizeParamInput(input, name);
  });
...
const DISTRIBUTIONS = [
  ...
  },
  {
    ...
    label: "Uniform",
    params: [
      { key: "u1", label: "a", type: "float", tip: "Minimum value" },
      { key: "u2", label: "b", type: "float", tip: "Maximum value" }
    ]
  },
  {
    key: "normal",
...
async function newPrompt() {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 25; attempt++) {
      try {
        data = await generateDistribution(allowed);
        break;
      } catch (err) {
        lastErr = err;
        console.warn(`Prompt generation failed on attempt ${attempt}`, err);
      }
    }

    if (!data) {
      promptEl.textContent = "Could not generate a valid prompt.";
      return;
    }
...
function normalizeParamInput(input, paramKey) {
  ...
  const param = dist?.params.find(p => p.key === paramKey);
  ...
  if (param.type === "int") {
    value = roundInt(value, 0, Infinity);
  }
  ...
  if (paramKey === "l" || paramKey === "a" || paramKey === "b" || paramKey === "s") {
...
function checkAnswer(selectedKey, userParams) {
  ...
  for (const p of dist.params) {
    ...
    const paramIsWrong =
      got === undefined ||
      (p.type === "int" && got !== expected) ||
      (p.type !== "int" && !nearlyEqual(got, expected));
    ...
    if (paramIsWrong) {
      if (!usedParamRetry) {
        return {
          correct: false,
          paramRetry: true,
          message: `The distribution is <strong>correct</strong>, but the parameters are <strong>incorrect</strong>. Try again!`
        };
      }
