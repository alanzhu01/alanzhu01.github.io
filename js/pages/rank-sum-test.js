...
import * as RankSum from "../math/rank-sum-test.js";
...
async function maybeUpdatePX() {
  ...
  const data = RankSum.rankSumProb(n1, n2, x, rel);
...
async function maybeUpdateX() {
  ...
  const data = RankSum.rankSumInverse(n1, n2, px, rel);
  xInput.value = data.x;
  return data.x;
}
...
async function maybeGeneratePlot() {
  ...
  const data = RankSum.rankSumData(n1, n2);
