...
import * as Uniform from "../math/uniform.js";
...
async function updateStats(a, b) {
  ...
  const data = Uniform.uniformStats(a, b, formula);
...
async function maybeUpdatePX() {
  ...
  if (!Number.isFinite(a) || !Number.isFinite(b) || !Number.isFinite(x) || a >= b) {
    pxOutput.value = "";
    return;
  }

  const data = Uniform.uniformProb(a, b, x, rel);
...
async function maybeUpdateX() {
  ...
  if (!Number.isFinite(a) || !Number.isFinite(b) || !Number.isFinite(px)) {
    xInput.value = "";
    return;
  }

  const data = Uniform.uniformInverse(a, b, px, rel);
  xInput.value = data.x;
  return data.x;
}
...
async function maybeGeneratePlot() {
  ...
  const data = Uniform.uniformData(a, b);
