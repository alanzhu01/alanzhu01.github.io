...
import * as ChiSquare from "../math/chi-square.js";
...
async function updateStats(n) {
  ...
  const data = ChiSquare.chisquareStats(n, formula);
...
async function maybeUpdatePX() {
  ...
  const data = ChiSquare.chisquareProb(n, x, rel);
...
async function maybeUpdateX() {
  ...
  const data = ChiSquare.chisquareInverse(n, px, rel);
  xInput.value = data.x;
  return data.x;
}
...
async function maybeGeneratePlot() {
  ...
  const data = ChiSquare.chisquareData(n);
