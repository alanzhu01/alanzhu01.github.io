...
import * as NegativeBinomial from "../math/negative-binomial.js";
...
async function maybeUpdatePX() {
  ...
  const data = NegativeBinomial.negbinProb(n, p, x, rel);
...
async function maybeUpdateX() {
  ...
  const data = NegativeBinomial.negbinInverse(n, p, px, rel);
  xInput.value = data.x;
  return data.x;
}
...
async function maybeGeneratePlot() {
  ...
  const data = NegativeBinomial.negbinData(n, p);
