...
import * as SignedRank from "../math/signed-rank-test.js";
...
async function updateStats(n1) {
  const formula = formulaToggle.checked ? 1 : 0;
  const data = SignedRank.signedRankStats(n1, formula);
...
function normalizeX() {
  ...
  const n = Number(n1Input.value);
  const raw = Number(xInput.value);

  if (!Number.isFinite(n)) {
    setError(xInput, "Enter a valid n first");
    return false;
  }

  const maxW = n * (n + 1) / 2;

  if (!Number.isInteger(raw) || raw < 0 || raw > maxW) {
    setError(xInput, `W must be an integer between 0 and ${maxW}`);
    return false;
  }
...
async function maybeUpdatePX() {
  ...
  if (!Number.isFinite(n1) || !Number.isFinite(x)) {
    pxOutput.value = "";
    return;
  }

  const data = SignedRank.signedRankProb(n1, x, rel);
...
async function maybeUpdateX() {
  ...
  if (!Number.isFinite(n1) || !Number.isFinite(px)) {
    xInput.value = "";
    return;
  }

  const data = SignedRank.signedRankInverse(n1, px, rel);
  xInput.value = data.x;
  return data.x;
}
...
async function maybeGeneratePlot() {
  ...
  await updateStats(n1);

  const data = SignedRank.signedRankData(n1);
    ...
    [
      {
        ...
        hovertemplate: "P(W=%{x}) = %{y:.6f}<extra></extra>"
      }
    ], 
    {
      title: null,
      ...
      xaxis: { 
        title: { text: "w", font: { size: axisTitleFont } },
      ...
      yaxis: { 
        title: { text: "P(W = w)", font: { size: axisTitleFont } },
        tickfont: { size: tickFont },
        showgrid: false,
        fixedrange: true,
        color: Utils.cssVar("--text-main"),
...
formulaToggle.addEventListener("change", () => {
  const n1 = parseInt(n1Input.value, 10);
  if (Number.isFinite(n1)) updateStats(n1);
});