...
function genValue(rootField, dist, params, template) {
  ...
  if (dist === "uniform") {
    if (rootField === "u1") return Number(uniform(1, 100).toFixed(2));

    if (rootField === "u2") {
      return Number(uniform(params.u1 + 1, params.u1 + 100).toFixed(2));
    }
  }
  ...
  if (dist === "beta") {
    if (rootField === "a_display") {
      if (params.a !== undefined) return ordinal(params.a);
